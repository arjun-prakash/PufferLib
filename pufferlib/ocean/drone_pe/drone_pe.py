...
import pufferlib
from pufferlib.ocean.drone_pe import binding

class DronePE(pufferlib.PufferEnv):
    def __init__(
        self,
        num_envs=16,
        num_drones=3,  # Default to 1 evader + 2 pursuers
        render_mode=None,
        ...
        self.single_observation_space = gymnasium.spaces.Box(
            low=-1,
            high=1,
            shape=(35,),  # Reduced from 41 since we removed 6 ring observations
            dtype=np.float32,
        )
...
def test_performance(timeout=10, atn_cache=1024):
    env = DronePE(num_envs=100, num_drones=3)  # Test with smaller setup
    env.reset()
    tick = 0
